...
from repositorios.delito import DelitoRepositorio
class DelitoController(BaseService):
    repository = DelitoRepositorio()
    def __init__(self, app):
        super().__init__('delito')
        delito_blueprint = Blueprint('delito', __name__)
        delito_blueprint.add_url_rule('/', view_func=self.getAll_route, methods=["GET"])
        delito_blueprint.add_url_rule('/<int:id>', view_func=self.getById_route, methods=["GET"])
        delito_blueprint.add_url_rule('/', view_func=self.create_raoute, methods=["POST"])
