...
from repositorios.condena import CondenaRepositorio
class CondenaController(BaseService):
    repository = CondenaRepositorio()
    def __init__(self, app):
        super().__init__('condena')
        condena_blueprint = Blueprint('condena', __name__)
        condena_blueprint.add_url_rule('/', view_func=self.getAll_route, methods=["GET"])
        condena_blueprint.add_url_rule('/<int:id>', view_func=self.getById_route, methods=["GET"])
        condena_blueprint.add_url_rule('/', view_func=self.create_raoute, methods=["POST"])
